...
;(function ( $, window, undefined ) {
  ...
  function Pep( el, options ) {
	...
	this.elementsWithInteraction = ['select', 'input', 'file'];
  ...
  Pep.prototype.subscribe = function () {
	...
	// Prevent start events from being gobbled by elements that should allow user interaction
    if(typeof(this.elementsWithInteraction) == "string") {
		this.$el.on( this.startTrigger, this.elementsWithInteraction, function(ev){
			ev.stopPropagation();
		});
	}
	else {
		for(var key in this.elementsWithInteraction) {
			this.$el.on( this.startTrigger, this.elementsWithInteraction[key], function(ev){
				ev.stopPropagation();
			});
		}
	}
