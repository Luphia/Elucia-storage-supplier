...
module.exports = {
	...
	post: function(_req, _res) {
		var client_id = _req.session.loginData.clientId, // 使用者帳號
			...
			file_info = _req.files; // 上傳檔案之資料
			partition_file_path = _req.params[0], // 檔案路徑
			remoteIP = _req.headers['public-ip'] || _req.connection.remoteAddress, // client ip
			isRepair = 0,
			rtdata = {},
			todoList = {};
			
		if(typeof(_req.headers.isrepair) != "undefined" ){
			isRepair = _req.headers.isrepair;
		}
		if(typeof(_req.headers.client_id) != "undefined" ){
			client_id = _req.headers.client_id;
		}			
		...
		// 確保 execFileDispatch 不會無窮迴圈
		// if(typeof(_req.headers.isReplicate) != "undefined" ){
			// isReplicate = _req.headers.isReplicate;
		// }		
		// if(isReplicate == 0){
			// todoList = [
				// [{"command": "checkToken", "progress": 25}],
				// [{"command": "execUpload", "progress": 25}],
				// [{"command": "returnUploadResult", "progress": 25}],
				// [{"command": "execFileDispatch", "progress": 25}]
			// ];
		// }else{
			// todoList = [
				// [{"command": "checkToken", "progress": 33}],
		...
		var cCommand = require( '../modules/command.js' )
			...
			"todoList" : [
				[{"command": "checkToken", "progress": 25}],
				[{"command": "checkHWLimit", "progress": 25}],
				[{"command": "execUpload", "progress": 25}],
				[{"command": "returnUploadResult", "progress": 25}],
				[{"command": "execFileDispatch", "progress": 25}]
			],
